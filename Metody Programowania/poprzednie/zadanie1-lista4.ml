...
module type HUFFMAN = functor (D : DICT) (PQ : PRIO_QUEUE) -> sig
  type 'a code_tree
  type 'a code_dict
...
module Huffman : HUFFMAN   = functor (D : DICT) (PQ : PRIO_QUEUE) -> struct
    ...
    let rec it xs d =
      ...
      | x :: xs' -> 
        it xs' (D.insert x 
                  (1 + find_else x 0 d) d)
    in it xs D.empty
  let code_tree xs = xs |> freq_dict|> D.to_list |> initial_pq |> algo
